...
class EmbeddingService:
    def __init__(self):
        ...
        self.client = OpenAI()
        logger.info("EmbeddingService initialized")  # Added

    def _embed(self, text: str):
        logger.debug("Requesting embedding")  # Added
    ...
    def create_embeddings_for_chunks(self, chunk_ids: list[uuid.UUID]):
        logger.info(f"Creating embeddings for {len(chunk_ids)} chunks")  # Added
        created = []

        for chunk_id in chunk_ids:
            chunk = self.chunk_repository.get_by_id(str(chunk_id))
            logger.debug(f"Embedding chunk {chunk_id}")  # Added
            ...
            logger.debug(f"Saved embedding {saved.id} for chunk {chunk_id}")  # Added
            created.append(saved)

        logger.info(f"Created {len(created)} embeddings")  # Added
        return created

    def process_user(self, user_id: uuid.UUID):
        logger.info(f"Processing embeddings for user {user_id}")  # Added

        docs = self.document_repository.get_by_user(str(user_id))
        doc_ids = [d.id for d in docs]
        logger.info(f"Found {len(doc_ids)} documents for user {user_id}")  # Added

        all_chunks = []
        for doc_id in doc_ids:
            logger.info(f"Creating chunks for document {doc_id}")  # Added
            chunks = self.chunk_service.create_chunks_for_document(doc_id)
            all_chunks.extend(chunks)

        logger.info(f"Created {len(all_chunks)} chunks for user {user_id}")  # Added

        chunk_ids = [c.id for c in all_chunks]
        embeddings = self.create_embeddings_for_chunks(chunk_ids)

        logger.info(f"Created embeddings for user {user_id}")  # Added
